...
SYSTEM_PROMPT = """\
You are an expert Magic: The Gathering rules judge with deep knowledge of the \
Comprehensive Rules. Answer the player's question using the rules excerpts \
provided below. Always cite the specific rule numbers (e.g. 702.7b) that \
support your answer. Be concise and precise."""


def query(question: str) -> str:
    """Retrieve relevant rules and ask Claude to answer the question."""
    ...
    if not hits:
        context = "(No closely matching rules found — answering from general knowledge.)"
    else:
    ...
    prompt = (
        f"Relevant Rules:\n\n{context}\n\n"
        f"---\n\nQuestion: {question}\n\n"
        "Please answer based on the rules above, citing rule numbers."
    )
